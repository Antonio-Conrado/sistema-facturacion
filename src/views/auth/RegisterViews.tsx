...
export default function RegisterViews() {
    ...
    const { register, reset, handleSubmit, watch, formState: { errors } } = useForm<AuthForm>({
        defaultValues: {
            email: '',
            password: ''
        }
    ...
    const handleData = (formData: AuthForm) => {
        const {repeatPassword, ...data} = formData//omit repeatPassword
        mutate(data)
    }

    return (
        <div className="bg-white rounded-md py-10 px-5">
            <h2 className="text-2xl text-center pb-4 text-cyan-700">Registrar usuario</h2>
